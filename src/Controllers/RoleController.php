...
class RoleController extends Controller
{
    ...
    {
        if (request()->ajax()) {
            $permissions_count = app(config('asimshazad.models.permission'))->count();
            $roles = app(config('asimshazad.models.role'))->withCount('permissions');
            $datatable = datatables($roles)
                ...
                ->editColumn('actions', function ($role) {
                    return view('asimshazad::roles.datatable.actions', compact('role'));
                })
                ->rawColumns(['actions']);
        ...
        return view('asimshazad::roles.index', compact('html'));
    }

    public function createForm()
    {
        $group_permissions = app(config('asimshazad.models.permission'))->all()->groupBy('group');

        return view('asimshazad::roles.create', compact('group_permissions'));
    }
    ...
    public function create()
    {
        ...
        $role = app(config('asimshazad.models.role'))->create(request()->all());
    ...
    {
        $role = app(config('asimshazad.models.role'))->findOrFail($id);

        return view('asimshazad::roles.read', compact('role'));
    }
    ...
    public function updateForm($id)
    {
        ...
        return view('asimshazad::roles.update', compact('role', 'group_permissions'));
    }
