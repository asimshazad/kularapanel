...
class CrudWidget extends Command
{
    ...
    public function setSimpleReplaces()
    {
        ...
        $this->replaces = [
            '{widgets_namespace}' => $controller_namespace = ltrim(config('asimshazad.widgets_namespace'),'\\'),
    ...
    public function makeDirectories()
    {
        ...
        $directories = [
            config('asimshazad.widgets_path'),
    ...
    {
        // create widget file
        $widget_file = config('asimshazad.widgets_path') . '/' . $this->replaces['{model_class}'] . 'Widget.php';
        if ($this->prompt($widget_file)) {
            $widget_stub = $this->files->get($this->asimshazad['stubs'] . '/widget.stub');
